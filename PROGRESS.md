...
| R5.02 | At least one useful debug visualization mode exists, such as particle view, density view, or surface view. | Verified | `physics_sim_ui_tests` verifies the visual-mode cycle helpers, and `src\main.cpp` renders mixed, density, and particle views without changing simulation state. | None. |
...
| R5.09 | Regression capture exists for at least one golden scene. | Verified | `physics_sim_demo_regression` compares a deterministic capture with `regression\demo_scene_golden.bmp`. | None. |
...
| R12.03 | User settings can persist preferences such as window size, overlay mode, and visual mode. | Verified | `docs\user-settings.md` documents the file path and format, `physics_sim_user_settings_tests` covers save/load and invalid/default handling, and the two-run manual check confirmed persisted window size, help overlay, and visual mode in the second startup log. | None. |
| R12.04 | A one-command local verification bundle runs tracking, build, tests, smoke, and relevant regression checks. | Verified | `.\scripts\verify-all.ps1` passed on 2026-06-04 and ran tracking validation, build, tests, smoke, and `scripts\verify-replay-suite.ps1`. | None. |
...
| R13.03 | Particle-grid transfer conserves mass and momentum and supports APIC/FLIP. | Verified | `physics_sim_water_tests` covers APIC affine velocity, mass-weighted scatter momentum conservation, and PIC/FLIP blending; `.\scripts\measure-water-solver.ps1 -Tier All` passed on 2026-06-05 with live `small-container-stress` at `6.61s` and `demo-grid-flow` at `0.56s`, and offline `small-container-stress` at `6.82s` and `demo-grid-flow` at `0.62s`. | None. |
| R13.04 | Pressure projection enforces incompressibility with measured residuals. | Verified | Matrix-free PCG projection reports iterations, residuals, convergence, and active pressure cells; `physics_sim_water_tests`, `.\scripts\test.ps1`, `.\scripts\verify-fluid-quality-suite.ps1`, and `.\scripts\measure-water-solver.ps1 -Tier All` passed on 2026-06-05. | None. |
| R13.05 | Dense particle clusters redistribute through density constraints without mass loss. | Verified | `physics_sim_water_tests` and `physics_sim_fluid_quality_tests` verify the bounded density-correction primitive and the overcrowding scenario reduce density error while preserving mass, volume, and deterministic center-of-mass behavior. | None. |
| R13.06 | World boundaries implement defined free-slip/no-slip material interaction. | Verified | `docs\boundary-interaction.md` defines the near-free-slip no-penetration wall material; `.\scripts\build.ps1`, `.\scripts\test.ps1`, `.\scripts\verify-fluid-quality-suite.ps1`, `.\scripts\verify-replay-suite.ps1`, and `.\scripts\check-tracking.ps1` passed on 2026-06-05. | None. |
...
| R13.08 | Particle resampling and rendering preserve physical volume. | Verified | `physics_sim_water_tests`, `physics_sim_fluid_quality_tests`, `.\scripts\verify-demo-scene.ps1`, `.\scripts\verify-demo-scene-density.ps1`, `.\scripts\verify-replay-suite.ps1`, and `.\scripts\verify-all.ps1` pass with the new deterministic resampling and volume-fraction rendering baselines. | None. |
| R13.09 | Live and offline physics-quality budgets pass deterministically. | Verified | `physics_sim_solver_benchmark`, `.\scripts\measure-water-solver.ps1 -Tier All`, `.\scripts\verify-fluid-quality-suite.ps1`, and `.\scripts\verify-all.ps1` passed on 2026-06-05 with live budgets inside `docs\performance-budget.md` and offline results recorded there as well. | None. |
