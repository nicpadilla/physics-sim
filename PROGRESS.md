...
| R6.02 | The app can create or load a scene, emit water, interact with walls, and show expected contained-water behavior. | Verified | PSIM-0087 added `scenes\starter_basin.pscene` plus transient pointer-water emission; `physics_sim_water_tests`, `physics_sim_scene_gallery_tests`, `physics_sim_tutorial_scene_tests`, `.\scripts\test.ps1` (26/26), and `.\scripts\verify-demo-scene.ps1` passed on 2026-06-06. | None. |
...
| R6.06 | The core workflow is complete: launch app, draw or load walls, place a fixture, observe water behavior, reset, repeat. | Verified | Pointer water is the default launch tool while `1` through `9` still select the editor tools; `physics_sim_editor_tests`, `physics_sim_player_guidance_tests`, `.\scripts\test.ps1` (26/26), and `.\scripts\run-smoke.ps1` passed on 2026-06-06. | None. |
...
| R8.06 | Solver performance has a documented budget and a repeatable profiling or timing check. | Verified | `docs\performance-budget.md` documents the fast, balanced, and quality budgets, and `.\scripts\measure-water-solver.ps1 -Profile All` passed on 2026-06-06. | None. |
...
| ID | Requirement | Status | Evidence | Next task or blocker |
| --- | --- | --- | --- | --- |
| R11.01 | Creative mode has a coherent start, edit, simulate, reset, and continue flow. | Verified | The app now starts from the no-spout starter basin with pointer water active by default; README/help overlay updates plus `physics_sim_player_guidance_tests`, `.\scripts\test.ps1` (26/26), and `.\scripts\run-smoke.ps1` passed on 2026-06-06. | None. |
...
| R11.07 | The default demo experience teaches the core sandbox loop through interaction rather than external explanation. | Verified | Normal and tutorial startup no longer include an authored spout, so the first water source is the pointer-water interaction; `physics_sim_tutorial_scene_tests`, `physics_sim_player_guidance_tests`, `.\scripts\test.ps1` (26/26), and `.\scripts\run-smoke.ps1` passed on 2026-06-06. | None. |
...
| ID | Requirement | Status | Evidence | Next task or blocker |
| --- | --- | --- | --- | --- |
| R13.01 | Fluid physics model, units, invariants, and equations are documented. | Verified | `docs\physics-accuracy-contract.md` and ADR 0006 define the model, units, invariants, and profile budgets; `.\scripts\check-tracking.ps1` and `.\scripts\test.ps1` passed on 2026-06-06. | None. |
...
| R13.04 | Pressure projection enforces incompressibility with measured residuals. | Verified | Matrix-free PCG projection reports iterations, residuals, convergence, visible cells, pressure-active cells, RHS norm, solution norm, `dt`, and rest density; `physics_sim_water_tests`, `.\scripts\test.ps1`, `.\scripts\verify-fluid-quality-suite.ps1`, `.\scripts\measure-water-solver.ps1 -Profile All`, and `.\scripts\verify-all.ps1` passed on 2026-06-06. | None. |
| R13.05 | Dense particle clusters redistribute through density constraints without mass loss. | Verified | `physics_sim_water_tests` and `physics_sim_fluid_quality_tests` verify the bucketed density-correction primitive and the overcrowding/long-run scenarios reduce density error while preserving mass, volume, and deterministic center-of-mass behavior; `.\scripts\test.ps1`, `.\scripts\verify-fluid-quality-suite.ps1`, `.\scripts\measure-water-solver.ps1 -Profile All`, and `.\scripts\verify-all.ps1` passed on 2026-06-06. | None. |
...
| R13.07 | Viscosity, surface tension, and free-surface behavior are modeled and tested. | Verified | `physics_sim_water_tests` and `physics_sim_fluid_quality_tests` cover bounded viscosity and surface-tension behavior, still-pool jitter, dam-break flow, and hose-wall stability; `.\scripts\test.ps1`, `.\scripts\verify-fluid-quality-suite.ps1`, and `.\scripts\verify-all.ps1` passed on 2026-06-06. | None. |
| R13.08 | Particle resampling and rendering preserve physical volume. | Verified | `physics_sim_water_tests`, `physics_sim_fluid_quality_tests`, `.\scripts\verify-demo-scene.ps1`, `.\scripts\verify-demo-scene-density.ps1`, `.\scripts\verify-demo-scene-surface.ps1`, `.\scripts\verify-replay-suite.ps1`, and `.\scripts\verify-all.ps1` passed on 2026-06-06 with deterministic resampling and volume-fraction rendering baselines. | None. |
| R13.09 | Live and offline physics-quality budgets pass deterministically. | Verified | `physics_sim_solver_benchmark`, `.\scripts\measure-water-solver.ps1 -Profile All`, `.\scripts\verify-fluid-quality-suite.ps1`, `.\scripts\verify-replay-suite.ps1`, and `.\scripts\verify-all.ps1` passed on 2026-06-06 with fast, balanced, and quality budgets inside `docs\performance-budget.md`. | None. |
