...
$result = Join-Path $repoRoot 'build\windows-x64\demo_scene_surface_regression-result.json'
...
{
    Write-Log "[demo-regression-surface] hash mismatch; capture preserved: $capture; result: $result"
    throw "Surface demo scene regression mismatch.`nBaseline: $baselineHash`nCapture:  $captureHash`nArtifacts: $capture, $result"
}
...
{
    Remove-Item -LiteralPath $capture -Force
    Write-Log "[demo-regression-surface] successful capture removed"
}
